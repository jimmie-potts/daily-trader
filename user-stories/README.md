...
- Migration `0004_portfolio_monitoring.sql` implements complete-cycle membership, request evidence, terminal-state and promotion guards, projection/reconciliation persistence, the current pointer, and worker lease/status without changing the earlier phase migrations. Additive migration `0005_portfolio_mleg_orders.sql` moves current snapshot/order writes to schema v2 for constrained unsupported multi-leg parent and child facts while retaining byte- and identity-stable reads of legacy v1 snapshots and orders.
...
- `npm run verify:phase4` passed the credential-free matrix: root CI with 74 test files/992 tests, dependency audit, sanitized fixture verification, clean migrations plus a seeded migrations-0001-through-0003 upgrade and nullable multi-leg order constraint checks, complete-cycle persistence, exact projection and reconciliation, all read-only API resources including past-end pages, dashboard build/tests, an incomplete cycle across a real service restart, orphan recovery, last-good pointer preservation, deterministic presentation, and bounded cleanup without deleting named volumes. The separate provider smoke remains P4-11's only open evidence.
