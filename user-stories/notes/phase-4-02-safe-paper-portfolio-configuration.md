...
The configuration rejects changed endpoint components, redirects, live-broker settings, execution enablement, partial credentials, unknown `PORTFOLIO_`, `PAPER_BROKER_`, or `LIVE_BROKER_` settings, and incompatible operational bounds before startup. Polling, request/page/response limits, retry and jitter, staleness, statement timeout, lease, and shutdown settings are validated once. Omitted order and fill item limits derive independently as the smaller of their preferred default and one less than their selected page budget. Explicit limits are never silently clamped, and every effective limit remains strictly below its budget so pagination can observe a short terminal page before the request bound is exhausted. Safe diagnostics expose only bounded settings, fixed read-resource names, and configuration-presence booleans; they omit URLs, credentials, account identifiers, and fingerprints.

## Validation Evidence

- `npm test --workspace=@daily-trader/config`: **Pass** - 75 tests covered disabled and page-budget-derived defaults, valid enabled configuration, endpoint and redirect rejection, separate credentials, expected-account requirements, live/execution rejection, unknown variables, explicit page/item and timing relationships, immutability, and redaction.
