...
- `npm test --workspace=@daily-trader/api`: **Pass** - 3 test files/40 tests covered complete, empty, stale, failed-sync, expired database-clock leases, missing reconciliation, suppressed and unsupported state, nullable multi-leg order facts, subsequent fill queries, all four GET endpoints, configured-capacity and past-end pagination, stable ordering, database failure, version rejection, and non-GET rejection.
