...
- A reusable contract suite verifies lifecycle ordering, cancellation, error classification, and exactly one classification or emission for each decoded frame item while preserving intentionally scripted provider duplicates. No event is emitted before authentication and subscription acknowledgement.
