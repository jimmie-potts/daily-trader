...
- Observed orders retain only immutable normalized provider facts required for monitoring, including side, exact quantities and prices when present, status, type, time in force, and lifecycle timestamps. An explicit nested `mleg` parent may lack singular asset, symbol, asset-class, or side facts; those values remain null, its instrument remains null, and it is visibly unsupported as `unsupported_order_structure` rather than being inferred from child legs. Concrete child identity remains intact, and an absent, null, or empty child order type remains null under the same unsupported structure reason. Canonical child `orderClass: mleg` records provider-supplied structural containment even when the child reports simple or empty class; it does not invent child symbol, side, or type. Fill observations retain exact quantity and price, side, transaction time, and source relationship. The transaction time is execution evidence and is not compared with the adapter's distinct activity-creation query bounds because Alpaca does not return the per-fill creation timestamp used by those filters. Neither contract becomes a local order, approval, recommendation, or execution request.
